@include('layouts.admin_header')
            ...
            <div class="modal-content">
                ...
                <form id="form-bidinvite" class="form-bidinvite" novalidate data-parsley-validate>
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" class="btn btn-primary add-invite btn-sm">Add</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <!-- Edit Modal-->
    <div class="modal fade" id="inviteEditModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Edit Invite</h5>
                    <button class="close" type="button" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">×</span>
                    </button>
                </div>
                <form id="edit-form-bidinvite" class="edit-form-bidinvite" novalidate data-parsley-validate>
                    ...
                    <div class="modal-footer">
                        <button class="btn btn-secondary btn-sm" type="button" data-dismiss="modal">Cancel</button>
                        <button type="button" class="btn btn-primary update-invite btn-sm">Update</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <div class="modal fade" id="suspendBidModal" tabindex="-1" role="dialog" aria-labelledby="suspendUserModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Suspend Invite</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <p class="text-danger">Invite commission will be unavailable in internal system are you sure to suspend this commission?</p>
                </div>
                <div class="modal-footer">
    ...
    $(document).ready(function(){
        ...
        bidTbl = $('#bid-table').dataTable({
            ...
            bPaginate: true,
            ordering:false,
            language: { search: "", searchPlaceholder: "Search invite" }
        });
    ...
    $(document).on('click', '.add-invite', function(){
        $('.error-add').text('');
        $(document).find('#form-bidinvite').parsley().whenValidate({
            group: 'block-1'
        }).done(function(){
            $('#add-invite').attr('disabled',true);
            $('#add-invite').text('Adding');
			var from 		    = $('.from').val();
			var to 		        = $('.to').val();
			...
			$.ajax({
                ...
                success: function(data) {
                    ...
                    if( d.success == 1 ) {
						$('#add-invite').attr('disabled',false);
    ...
    $(document).on('click', '.edit-bidinvite', function() {
        ...
        $('.update-invite').attr('data-id', bid_id);
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $('#inviteEditModal').modal('show');
            }   
        });

    });

    $(document).on('click', '.update-invite', function(){
        $('.error-edit').text('');
        $(document).find('#edit-form-bidinvite').parsley().whenValidate({
            group: 'block-2'
        }).done(function(){
            $('#update-invite').attr('disabled',true);
            $('#update-invite').text('Updating');
            var id          = $('.update-invite').attr('data-id');
			var from 		= $('.efrom').val();
			var to 		    = $('.eto').val();
			...
			$.ajax({
                ...
                success: function(data) {
                    ...
                    if( d.success == 1 ) {
                        bidTbl.api().ajax.reload();
                        $('#inviteEditModal').modal('hide');
						toastr.success('Updated Successfuly!');
                    } else {
                        $('#update-invite').attr('disabled',false);
                        $('#update-invite').text('Update');
